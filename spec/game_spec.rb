...
RSpec.describe 'Game#play' do
  ...
  let(:player1) { double('Player', name: 'P1', token: 'X', make_move: '1') }
  let(:player2) { double('Player', name: 'P2', token: 'O', make_move: '2') }
  ...
  it 'asks first player to pick new position' do
    ...
    expect(player1).to receive(:make_move).and_return('1')

    game.play
  end

  it 'prompts player for new input if input is bad' do
    allow(player1).to receive(:make_move).and_return('10', 'x', '9')
